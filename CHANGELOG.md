...
- **`make verify-release` now fails closed.** Its last block chained unzip, the
  ...
  0 and the upload proceeded. Each step is now judged on its own and sets a
  return code, the packaged binary's `--version` must contain the tag being
  released (a zip left over from another tag passes the notarization gates and
  fails here), and only the informational `spctl` line may be ignored. Matches
  the org template (CONVENTIONS.md §Code Signing → Verifying a release).
...
- **darwin/amd64 (Intel) pre-built binary.** macOS releases now ship
  **arm64 only**, per the org-wide policy (darwin is Apple-Silicon only; no
  universal binaries). Intel Mac users can build from source (the darwin
  build still uses `CGO_ENABLED=1` for OS Keychain access).
...
- **darwin code-signature identifier** is now the canonical `scli`
  (was `scli-darwin-arm64`), set via `codesign -i` so it stays stable after
...
- **Darwin releases are now Developer ID signed and Apple-notarized.**
  `scli-v1.7.3-darwin-{amd64,arm64}.zip` carry full Apple Developer
  ...
  users who place `scli` under Dropbox-synced (or any other
